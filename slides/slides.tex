...
% Centered figure that can never exceed a set fraction of the frame height,
% so a figure + a few bullets always fit. Default 0.6; pass e.g. [0.72] for
% figure-only frames or [0.5] for text-heavy ones.
\newcommand{\slidefig}[2][0.6]{%
  \begin{center}%
...
\begin{frame}[t]{Median Salary by Gender, FY2015--2024}
  \slidefig[0.6]{salary_trends}
  \begin{itemize}
...
\begin{frame}[t]{Female Share of Federal Economists by Agency}
  \slidefig[0.56]{female_share_agency}
  \begin{itemize}
...
\begin{frame}[t]{Is the Gap Within or Between Grade?}
  \slidefig[0.58]{grade_salary}
  \begin{itemize}
...
\begin{frame}[t]{Salary Gap by Agency}
  \slidefig[0.5]{agency_gap}
  \begin{itemize}
...
\begin{frame}[t]{Federal Salary Gap: FedsDataCenter vs.\ Foster et al.\ (2020)}
  \slidefig[0.78]{salary_comparison_paper}
\end{frame}
...
% =============================================================================
\section{The 2025 Update}
% =============================================================================

\begin{frame}{The 2025 Update: Data}
  \begin{itemize}
    \item Question: has the \textbf{number} and \textbf{pay} of federal
          economists changed into 2025?
    \item FedsDataCenter stops at FY2024. 2025 uses \textbf{OPM FedScope}:
          preliminary \textbf{March 2025} snapshot with \textbf{September 2024}
          bundled for comparison (occupational series 0110).
    \item Three caveats up front:
      \begin{itemize}
        \item \textbf{No gender / race} --- removed from FedScope as of March
              2025 (EOs 14151/14168/14173). This update is headcount and pay only.
        \item \textbf{Preliminary}, and still counts administrative-leave /
              deferred-resignation staff as employed $\Rightarrow$ headcount loss
              understated.
        \item Different universe from FedsDataCenter (EHRI month-end status);
              small agency\,$\times$\,grade cells suppressed ($\sim$76\% coverage).
      \end{itemize}
  \end{itemize}
\end{frame}

\begin{frame}[t]{Number and Pay of Federal Economists}
  \slidefig[0.62]{fed_economists_2025}
  \vspace{-0.3em}
  \begin{itemize}\small
    \item Headcount \textcolor{myred}{$-2.2\%$} (4,998 $\to$ 4,888),
          Sep 2024 $\to$ Mar 2025.
    \item Mean pay $+2.8\%$ nominal, \textbf{$+1.4\%$ real} (CPI-U) --- almost
          entirely the January 2025 pay raise (2.0\%), not a shift to more
          senior staff.
  \end{itemize}
\end{frame}

\begin{frame}[t]{Hires and Departures: the Clearer Signal}
  \slidefig[0.6]{econ_flows_2025}
  \vspace{-0.3em}
  \begin{itemize}\small
    \item Quarterly net: $+21,\ +64,\ -23,\ \textcolor{myred}{-101}$ (2025\,Q1).
          Hiring collapsed (145 $\to$ 36); departures spiked (81 $\to$ 137).
    \item Leavers averaged \textbf{12.8 years} of service and \$145k ---
          mid-career departures, not junior attrition.
  \end{itemize}
\end{frame}

\begin{frame}[t]{Why They Left: RIF and Buyout, Not Ordinary Turnover}
  \slidefig[0.54]{econ_separation_reasons_2025}
  \vspace{-0.3em}
  \begin{itemize}\small
    \item 2025\,Q1 holds 86\% of the year's early-out retirements, 64\% of
          term-appointment endings, 42\% of voluntary retirements, 37\% of quits.
    \item Transfers to other agencies \textit{fell} --- economists left
          government rather than moving within it.
  \end{itemize}
\end{frame}

\begin{frame}[t]{Where Economists Were Gained and Lost}
  \slidefig[0.66]{econ_agency_change_2025}
  \vspace{-0.3em}
  \begin{itemize}\small
    \item Statistical agencies among the losers: \textbf{BLS $-24$, BEA $-14$};
          also Treasury $-13$, Agriculture $-13$, FTC $-9$.
    \item Gained: Energy $+17$, FDIC $+12$, DHS $+11$, HHS $+7$, Census $+6$.
  \end{itemize}
\end{frame}

\begin{frame}[t]{Historical Context: Economists Since 1998}
  \slidefig[0.66]{econ_headcount_since_1998}
  \vspace{-0.3em}
  \begin{itemize}\small
    \item On a consistent post-2006 basis, headcount ran 4,280--5,000. March
          2025 (4,888) is still near the \textit{top} of that range --- the drop
          so far unwinds about a year of growth, not decades.
    \item Real pay is mid-range: above the late 1990s, below the 2010 and
          2018--20 peaks.
  \end{itemize}
\end{frame}

\begin{frame}[t]{Historical Context: 2025 Outflow vs.\ Prior Years}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \includegraphics[width=\linewidth,height=0.5\textheight,keepaspectratio]{econ_separations_history}
    \end{column}
    \begin{column}{0.5\textwidth}
      \includegraphics[width=\linewidth,height=0.5\textheight,keepaspectratio]{econ_q1_history}
    \end{column}
  \end{columns}
  \begin{itemize}\small
    \item FY2024 was the calmest year on record (277 separations); FY2025's
          \textbf{first six months} already hit 215. Jan--Mar 2025 had the most
          departures and nearly the fewest hires of any Q1 since 2015.
    \item The one precedent --- FY2019, the ERS move to Kansas City --- was one
          agency. \textbf{No separation carries a formal RIF code}: the 2025
          drawdown runs through quits, retirements and term-appointment endings.
  \end{itemize}
\end{frame}

\begin{frame}{2025 Update: Robustness}
  \begin{center}
    \small
    \begin{tabular}{lrrrr}
      \toprule
      ...
      0110 with a doctorate (Foster proxy) & --- & 1,386 & --- & mean \$179k \\
      \bottomrule
    \end{tabular}
  \end{center}
  \vspace{0.3em}
  \small
  \begin{itemize}
    \item Result is not sensitive to the series definition.
    \item Foster et al.\ (2020, 2023) restrict to \textbf{PhD} federal
          economists --- $\approx$ the 1,386 doctorate holders here (28\% of
          0110). Explains their Treasury/Agriculture/HHS focus vs.\ our
          BLS-heavy full population.
    \item Shared universe exclusions: CEA, Federal Reserve Board, CBO, USPS,
...
\begin{frame}[t]{Placement Trends Over Time}
  \slidefig[0.6]{placement_trends}
  \begin{itemize}
...
\begin{frame}[t]{Placement Shares: EconPhDPlacements vs.\ Foster et al.\ (2023, JEP)}
  \slidefig[0.58]{placement_comparison}
\end{frame}
...
\begin{frame}[t]{Female Share by Sector: EconPhDPlacements vs.\ Foster et al.}
  \slidefig[0.56]{female_sector_comparison}
\end{frame}
...
\begin{frame}[allowframebreaks]{Summary}
  ...
  \end{itemize}
  \vspace{0.4em}
  \textbf{The 2025 update (OPM FedScope, Sep 2024 $\to$ Mar 2025):}
  \begin{itemize}
    \item Headcount $-2.2\%$; mean pay $+2.8\%$ nominal / $+1.4\%$ real
          (almost entirely the January 2025 raise)
    \item Flows show the turn: 2025\,Q1 net $-101$, hiring collapsing as
          mid-career economists (mean 12.8 yr service) depart
    \item BLS and BEA among the agencies losing economists
    \item No gender data after March 2025 (removed under 2025 executive orders)
  \end{itemize}
  \vspace{0.4em}
